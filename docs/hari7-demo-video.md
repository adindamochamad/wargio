...
- [ ] Browser zoom 100%, dark mode ON (sesuai UI demo)
...
**Aksi layar (ketik persis):**

1. Buka `https://wargio.adindamochamad.com`
2. Ketik: `stok indomie goreng berapa?`
3. Tunggu balasan (stok + status minimum)
4. **Split screen:** kiri chat, kanan Atlas → `products` → filter/search `Indomie Goreng` → tunjukkan `stock_current`, `stock_minimum`
...
1. Ketik: `tadi jual 2 indomie goreng dan 1 air mineral aqua 600ml`
2. Baca ringkasan konfirmasi di chat
3. Ketik: `ya`
4. Tunggu balasan sukses + total Rupiah
5. *(Opsional)* Atlas → `transactions` sort `created_at` desc → dokumen terbaru type `sale`
6. *(Opsional)* `products` → stok Indomie/Aqua turun

> Jangan pakai qty besar. Rehearsal dulu — jika stok kurang, kurangi jumlah di kalimat.
...
1. Ketik: `siapa yang belum bayar hutang minggu ini?`
2. Tunggu list customer + nominal
3. Ketik: `Bu Sari bayar hutang 50000`
4. Ketik: `ya` setelah konfirmasi
5. *(Opsional)* Atlas → `customers` → Bu Sari → `debt_total` berkurang
...
1. Ketik: `berapa pendapatan minggu ini?`
2. Scroll ke **dashboard** di kanan/bawah UI (stok kritis, hutang teratas, omzet hari ini)
3. *(Alternatif cepat)* Ketik: `kira-kira besok bakal ramai?` → forecast day-of-week

> Catatan jujur: intent "produk paling laris" belum ada; gunakan **sales_report** + dashboard — judge tetap lihat agregasi Atlas.
...
**Aksi layar:** fullscreen slide Gamma Architecture (Prompt 2). Bisa zoom perlahan ke panah MCP → Atlas.
...
**Aksi layar:** slide Closing (Prompt 3) + URL `wargio.adindamochamad.com` terbaca.
...
- Ketik **natural**, jangan paste (terlihat di video)
